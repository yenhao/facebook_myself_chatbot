...
def createTrainingMatrices(conversationFileName, wList, maxLen):
    ...
    for index,(key,value) in enumerate(conversationDictionary.items()):
        ...
        # keySplit = key.split()
        keySplit = list(jieba.cut(key, cut_all=False))
        # valueSplit = value.split()
        valueSplit = list(jieba.cut(value, cut_all=False))
...
def getTestInput(inputMessage, wList, maxLen):
    ...
    inputSplit = list(jieba.cut(inputMessage, cut_all=False))
...
if (os.path.isfile('wordList.txt')):
    ...
    with open('./data/qa.pickle', "rb") as fp:
        qa_pair = pickle.load(fp)
    sentence = ""
    for q,a in qa_pair:
        sentence += q[1] + a[1]
    wordList = list(Counter(jieba.cut(sentence, cut_all=False)))
